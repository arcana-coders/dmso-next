...
export default function PaymentSealBadges({ className = '', compact = false }: PaymentSealBadgesProps) {
  ...
  const badgeClass = 'h-full min-h-[68px] w-full rounded-xl border border-[#eef2f6] shadow-sm';
  ...
  return (
    <div className={`rounded-2xl border border-[#dfe7ef] bg-white ${padding} shadow-[0_12px_28px_rgba(0,63,135,0.06)] ${className}`}>
      ...
      <div className={`mx-auto grid ${maxWidth} grid-cols-2 gap-2 md:gap-2.5 md:grid-cols-4`}>
        <svg viewBox="0 0 180 88" role="img" aria-label="PayPal Verified" className={`${badgeClass} bg-[#fff8cc]`}>
          <defs>
            <path id="paypalSealShared" d="M90 8l5 5 7-3 4 6 8-1 2 7 7 1-1 8 6 4-3 7 5 5-5 5 3 7-6 4 1 8-7 1-2 7-8-1-4 6-7-3-5 5-5-5-7 3-4-6-8 1-2-7-7-1 1-8-6-4 3-7-5-5 5-5-3-7 6-4-1-8 7-1 2-7 8 1 4-6 7 3z" />
          </defs>
          <use href="#paypalSealShared" fill="#ffe16b" />
          <use href="#paypalSealShared" fill="none" stroke="#f0c83a" strokeWidth="2" />
          <text x="90" y="40" textAnchor="middle" fontSize="23" fontWeight="900" fill="#003087" fontStyle="italic">PayPal</text>
          <text x="90" y="57" textAnchor="middle" fontSize="10" fontWeight="900" fill="#0070ba" letterSpacing="3">VERIFIED</text>
        </svg>

        <svg viewBox="0 0 180 72" role="img" aria-label="McAfee Secure" className={`${badgeClass} bg-white`}>
        ...
        <svg viewBox="0 0 180 72" role="img" aria-label="Norton Secured" className={`${badgeClass} bg-white`}>
        ...
        <svg viewBox="0 0 180 72" role="img" aria-label="Google Trusted" className={`${badgeClass} bg-white`}>
