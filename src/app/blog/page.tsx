...
export default function BlogPage() {
    return (
        <main className="container mx-auto px-6 py-12">
            <h1 className="text-4xl font-bold text-dmso-dark mb-4 text-center">Blog DMSO</h1>
            <p className="text-center text-stone-600 max-w-2xl mx-auto mb-16 text-lg">
                Guías, artículos y contenido educativo sobre Dimetilsulfóxido.
                Descubre usos, protocolos de seguridad y contexto práctico para tomar mejores decisiones.
            </p>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                {articles.map((article) => (
                    <Link key={article.slug} href={`/blog/${article.slug}`} className="group flex flex-col bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-stone-100">
